...
class DialogueSpecialTokens:
    """Resolve the dialogue formatting tokens from the tokenizer vocab.

    Prefer the canonical cosy-dialogue tokens and only fall back to legacy Soul
    names when the canonical ones are unavailable.
    """
    ...
    def __init__(self, tokenizer):
        ...
        self.speaker_tokens = [
            self._resolve_token(f"<|SPEAKER_{i}|>")
            for i in range(8)
        ]
        self.sos = self._resolve_token("<|SOS|>", "[SOS]", required=False)
        self.text_start = self._resolve_token("<|start_text|>", "<|text_start|>")
        self.text_end = self._resolve_token("<|end_text|>", "<|text_end|>")
        self.task_id = self._resolve_token("<|task_id|>", "<|semantic_token_start|>")

    def _resolve_token(self, preferred, fallback=None, required=True):
        for token in (preferred, fallback):
            if token and token in self.vocab:
                return token
        if required:
            raise ValueError(
                f"Tokenizer is missing required dialogue token '{preferred}'"
                + (f" (fallback '{fallback}')" if fallback else "")
            )
        return ""

    def wrap_text(self, text: str, spk_idx: int, include_sos: bool = False) -> str:
        if spk_idx < 0 or spk_idx >= len(self.speaker_tokens):
            raise ValueError(f"Speaker index out of range: {spk_idx}")
        prefix = f"{self.speaker_tokens[spk_idx]}{self.text_start}"
        if include_sos and self.sos:
            prefix = f"{self.sos}{prefix}"
        return f"{prefix}{text}{self.text_end}{self.task_id}"

    def dialogue_prefix_tokens(self):
        if not self.sos:
            return []
        return self.tokenizer.encode(self.sos)
...
class PodcastDataset(Dataset):
    ...
    def __init__(self, text_tokenizer, data_list, model_config: Config):
        ...
        {"key": "uttid_1", "prompt_text": ["prompt_text1", "prompt_text2"], "dialect_prompt_text": ["dialect_prompt_text1", "dialect_prompt_text2"],
            "text": ["text1", "text2], "spk": [0, 1], "prompt_wav": ["data/audio/00000000.wav", "data/audio/00000001.wav"], "wav": "data/audio_synthetic/uttid_1.wav"}
        ```
        Note:
        ...
        self.dialogue_tokens = DialogueSpecialTokens(text_tokenizer)
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            dialect_prefix_list = [self.dialogue_tokens.dialogue_prefix_tokens()]
            for spk_idx, (prompt_text, prompt_wav) in enumerate(zip(data["prompt_text"], data["prompt_wav"])):
                ...
                prompt_text = self.dialogue_tokens.wrap_text(
                    prompt_text,
                    spk_idx=spk_idx,
                    include_sos=(spk_idx == 0),
                )
                ...
                if use_dialect_prompt:
                    ...
                    dialect_prompt_text = self.dialogue_tokens.wrap_text(
                        dialect_prompt_text,
                        spk_idx=spk_idx,
                        include_sos=False,
                    )
                    ...
                    if spk_idx == 0:
                        dialect_prefix_list.append(self.dialogue_tokens.dialogue_prefix_tokens())
                    else:
            ...
            for text, spk in zip(data["text"], data["spk"]):
                ...
                text = self.dialogue_tokens.wrap_text(text, spk_idx=spk, include_sos=False)
...
class PodcastInferHandler(PodcastDataset):
    ...
    def __init__(self, text_tokenizer, data_list, model_config: Config):
        ...
        """Example data_list:
        ```
        {"key": "uttid_1", "prompt_text": ["prompt_text1", "prompt_text2"], "dialect_prompt_text": ["dialect_prompt_text1", "dialect_prompt_text2"], "text": ["text1", "text2], "spk": [0, 1], "prompt_wav": ["data/audio/00000000.wav", "data/audio/00000001.wav"], "wav": "data/audio_synthetic/uttid_1.wav"}
        ```
        Note:
            - `key` is the key of this sample.
            - `prompt_text` is the text used for prompt.
